...
namespace distro
{
    public partial class LaporanPenjualankaryawan : Form
    {
        ...
        public LaporanPenjualankaryawan(pengguna akun)
        {
            InitializeComponent();
            InitializePrinting();
            _akun = akun;
                ...
                using (dbContext = new distroshopDataContext())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(keyword))
                    {
                        ...
                        query = query.Where(item =>
                            item.prod.Nama.ToLower().Contains(lowerKeyword) || 
                            item.pel.Nama.ToLower().Contains(lowerKeyword) || 
                            item.k.NamaKategori.ToLower().Contains(lowerKeyword) || 
                            (item.vp.Ukuran != null && item.vp.Ukuran.ToLower().Contains(lowerKeyword)) || 
                            (item.vp.Warna != null && item.vp.Warna.ToLower().Contains(lowerKeyword)) || 

                            // --- Pencarian Tanggal (tanggal lengkap, bulan, atau tahun) ---
                            (item.p.Tanggal.HasValue && isFullDate && item.p.Tanggal.Value.Date == searchDate.Date) || 
                            (item.p.Tanggal.HasValue && isYear && item.p.Tanggal.Value.Year == searchYear) || 
                            (item.p.Tanggal.HasValue && isMonthNumber && item.p.Tanggal.Value.Month == searchMonth) || 
                            (item.p.Tanggal.HasValue && isMonthName && item.p.Tanggal.Value.Month == monthFromName) || 
                            ...
                            // -- Angka: Coba cocokkan angka jika keyword bisa diparse sebagai angka
                            (isInt && item.dp.Jumlah == searchInt) ||
                            (isDecimal && item.dp.HargaSatuan == searchDecimal) ||
                            (isDecimal && item.dp.SubTotal == searchDecimal) ||
                            (isDecimal && item.p.TotalHarga == searchDecimal) || 
